...
# Generation date: 2026-08-23T10:28:50
...
class FileHashAlgorithm(EnumDefinitionImpl):
    ...
    sha256 = PermissibleValue(
        text="sha256",
        description="""SHA-256 is a secure cryptographic hash function that turns any input data into a fixed 64-character (256-bit) hexadecimal code. It is a one-way process, meaning you cannot reverse the output to find the original input.""")
    md5 = PermissibleValue(
        text="md5",
        description="""MD5 (Message-Digest Algorithm 5) is a function that turns any text or file into a unique 32-character code. It acts as a digital fingerprint to check if data is correct.""")
    xxh3_64 = PermissibleValue(
        text="xxh3_64",
        description="""xxh3_64 is the 64-bit variant of XXH3, an extremely fast, non-cryptographic hash algorithm from the xxHash family. It operates near RAM speed limits and outperforms older legacy variants like XXH64 and XXH32 across all input sizes, particularly on small data.""")
...
class FileRole(EnumDefinitionImpl):
    ...
    topology = PermissibleValue(
        text="topology",
        description="""Defines how atoms connect to form molecules and assigns physical properties like atomic masses, partial charges, and force field parameters for bonds, angles, and dihedrals.""")
    trajectory = PermissibleValue(
        text="trajectory",
        description="""Stores the sequential physical positions, velocities, forces, energies and other time-varying properties of atoms or molecules over the duration of a simulation.""")
    metadata = PermissibleValue(
        text="metadata",
        description="""Structured data file that describes the parameters, system composition, software versions, and provenance of a simulation experiment.""")
    log = PermissibleValue(
        text="log",
        description="""A text-based output file generated by simulation software that records a step-by-step text history, performance metrics, and thermodynamic property updates during a computational run.""")
    parameter = PermissibleValue(
        text="parameter",
        description="""Defines the numerical values, physical constraints, and algorithmic settings—such as time steps, temperature, pressure control, and run length—required to execute a computer simulation of atomic movements.""")
    other = PermissibleValue(
        text="other",
        description="Any other file content that isn't decribed by this list.")
